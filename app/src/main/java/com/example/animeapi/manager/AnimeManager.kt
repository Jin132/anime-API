...
class AnimeManager(context: Context) {
    ...
    fun getPopularAnimes(): List<Anime>? {
        return animeDao?.getAnimes()?.take(20)
    }
    ...
    private fun markAnime(item: Anime, labelName: String) {
        val user = userDao?.getActiveUser()
        val anime = animeDao?.getAnimeById(item.id)
        if (user != null && anime != null) {
            val existingAnime =
                userAnimeDao?.getAnimesByUserAnimeAndType(user.id, item.id, labelName)
    ...
    private fun saveAnimesToDb(animes: List<Anime>) {
        for (anime in animes) {
            val animeList = animeDao?.getAnimeLikeTitle(anime.animes)
